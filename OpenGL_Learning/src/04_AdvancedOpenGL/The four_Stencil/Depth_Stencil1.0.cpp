...
//
//   /* int stencilBits = 0;
//    glGetIntegerv(GL_STENCIL_BUFFER_BIT, &stencilBits);
//    std::cout << "GL_STENCIL_BUFFER_BIT = " << stencilBits << std::endl;*/
//
//	// 创建带有颜色纹理和深度 / 模板渲染缓冲区的帧缓冲对象（FBO） (注意这里的深度/模板缓冲是共用一个renderbuffer（FBO）的)
//    GLuint fbo, rbo;
...
//        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);// 清除颜色缓冲
//
//        SingleShader.use();
//        shader.use();
...
//        glDrawArrays(GL_TRIANGLES, 0, 36);
//
//        // 第二遍渲染通道：现在绘制物体的略微缩放版本，这次要禁用模板写入。
//        // 因为模板缓冲区现在填充了几个 1。缓冲区中值为 1 的部分不会被绘制，因此只绘制物体的大小差异，使其看起来像边界。
//		// 立方体边框
//		glStencilFunc(GL_NOTEQUAL, 1, 0xFF); // 设置模板测试函数
//		glStencilMask(0x00); // 禁止写入模板缓冲
...
//
//        // debug: 不用模板、关闭深度，直接绘制放大的立方体
//        glDisable(GL_STENCIL_TEST);
//        glDisable(GL_DEPTH_TEST);
//        SingleShader.use();
//        glm::mat4 dbgM = glm::translate(glm::mat4(1.0f), glm::vec3(-1.0f,0.0f,-1.0f));
//        dbgM = glm::scale(dbgM, glm::vec3(1.12f));
//        SingleShader.setMat4("model", dbgM);
//        glBindVertexArray(cubeVAO);
//        glDrawArrays(GL_TRIANGLES, 0, 36);
//		// 恢复使用模板和深度测试
//        glEnable(GL_DEPTH_TEST);
