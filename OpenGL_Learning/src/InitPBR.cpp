...
void InitPBR(float* data, int width, int height, Shader& equirectangularToCubemapShader, Shader& irradianceShader, Shader& prefilterShader, Shader& brdfShader)
{
    FramebufferInit();
    HDRenvLoad(data, width, height);
    CubemapSet();
    HDRConvert(equirectangularToCubemapShader);
    IrradianceMAP_Create_Init(irradianceShader);
    PrefilterMAP_Create_Init(prefilterShader);
    BrdfLUTTextureGenerate(brdfShader);
}
